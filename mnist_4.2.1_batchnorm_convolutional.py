...
from CustomDataSet import read_data_sets
...
#The prediction should be done using reasonable size batch of image.
#ResourceExhaustedError can occur if the prediction is done with too much data at the same time.
...
#datavis.animate(training_step, ITERATION+1, train_data_update_freq=20, test_data_update_freq=100)
...
#Training with minimum display to speed up the training (about 5 minutes on Lenovo Y510P Laptop with CUDA & CUDNN)
...
for i in range(0, len(mnist.test.images), PRED_BATCH_SIZE):
	pred = (prediction.eval(feed_dict={X: mnist.test.images[i:i+PRED_BATCH_SIZE], tst: False, pkeep: 1.0}, session = sess))
	for p in pred:
		predictions.append([off, int(p)])
		off = off + 1

# Save the prediction on Kaggle submission format.
numpy.savetxt("submit.csv", predictions, delimiter=",", fmt='%i', header="ImageId,Label", comments = "")
